...
from .limr_args import LIMRArguments # Import LIMRArguments
...
def get_train_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> _TRAIN_CLS:
    ...
    # Manually convert limr dict to LIMRArguments object if necessary
    if hasattr(training_args, "limr") and isinstance(training_args.limr, dict):
        logger.debug("Converting limr dictionary to LIMRArguments object.")
        # Filter out keys not present in LIMRArguments to avoid errors
        limr_fields = {f.name for f in fields(LIMRArguments)}
        filtered_limr_dict = {k: v for k, v in training_args.limr.items() if k in limr_fields}
        try:
            training_args.limr = LIMRArguments(**filtered_limr_dict)
        except Exception as e:
            logger.error(f"Failed to instantiate LIMRArguments: {e}")
            raise e
