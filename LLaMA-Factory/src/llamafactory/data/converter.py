...
class AlpacaDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        # --- Add ID with logging ---
        original_id_value = example.get("id", "MISSING_KEY") # Use get to avoid KeyError
        if original_id_value == "MISSING_KEY":
            logger.debug(f"Converter: 'id' key missing in example.") # Use debug level
            output["_id"] = None
        elif original_id_value is None:
            logger.debug(f"Converter: 'id' key found but value is None.") # Use debug level
            output["_id"] = None
        else:
            # logger.debug(f"Converter: Found id '{original_id_value}' (type: {type(original_id_value)}). Converting to string.") # Use debug level
            output["_id"] = str(original_id_value)
...
class SharegptDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        output = {
            ...
            "_videos": self._find_medias(example[self.dataset_attr.videos]) if self.dataset_attr.videos else None,
            "_audios": self._find_medias(example[self.dataset_attr.audios]) if self.dataset_attr.audios else None,
            # Preserve the original ID if it exists, converting to string
            "_id": str(example["id"]) if "id" in example and example["id"] is not None else None,
        }
        return output
