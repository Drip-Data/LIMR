...
from typing import TYPE_CHECKING, Any, Literal, Optional, Union # Import Union

import numpy as np
import torch
import torch.nn.functional as F
from transformers import DataCollatorForSeq2Seq, DataCollatorWithPadding # Add DataCollatorWithPadding import
...
# Define IGNORE_INDEX if not already defined or imported
# (Keep existing logic for IGNORE_INDEX definition)
try:
...
@dataclass
class DataCollatorWithPaddingAndIDs: # No inheritance as per plan
    """
    Data collator for PPO training that manually pads only 'input_ids'
    using tokenizer.pad (which also generates 'attention_mask'),
    and preserves the 'id' field. Ignores 'labels'.
    """
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, Any]:
        # Separate IDs first. Labels are not expected or processed.
        ids = [feature.pop("id", None) for feature in features] # Pop IDs

        # Extract only input_ids for padding
        # Ensure features actually contain 'input_ids'
        input_ids_list = [feature["input_ids"] for feature in features if "input_ids" in feature]

        # Handle empty batch or batch without input_ids
        if not input_ids_list:
            # Return IDs if they exist, otherwise an empty dict
            return {"id": ids} if ids else {}

        # Pad only input_ids. tokenizer.pad will create attention_mask.
        # Pass input_ids as a dictionary key, as expected by tokenizer.pad
        ...
        # Add the collected IDs back to the batch.
        # Ensure the length matches the padded batch size
        if ids and len(ids) == batch['input_ids'].size(0):
             batch["id"] = ids
        elif ids: # If IDs exist but length mismatch (shouldn't happen ideally)
             logger.warning_rank0(f"ID count ({len(ids)}) mismatch with padded batch size ({batch['input_ids'].size(0)}). Truncating IDs.")
             batch["id"] = ids[:batch['input_ids'].size(0)] # Add potentially truncated IDs

        # Labels are intentionally not processed or added.
